...
PI = 3.141592653
BOWLING_MASS = int(6) # mass of a bowling ball - for presets
BOULDER_MASS = int(45) # mass of a boulder - for presets
...
class Window(): #class of the main window where the simulation will run
    ...
    def initUI(self): # mostly the same as last step.
        global m1, m2, d
        ...
        presetMsg = tk.Label(main, text = "Or choose from the presets below:") # message to inform the user that they can select from presets instead
        presetMsg.place(x = 802, y = 450)
        
        preset1 = tk.Button(main, text = "2 BOWLING BALLS", relief = FLAT, bg = "#666", fg = "white") # buttons for preset selection
        preset1.place(x=802, y=490)
        preset1.config(command = lambda : self.choosePreset1(main, preset1, m1StatusMsg, m2StatusMsg, dStatusMsg)) # commands are set to the selection methods
    ...
    def choosePreset1(self, main, preset1, m1StatusMsg, m2StatusMsg, dStatusMsg): # choose presets methods
        global m1, m2, d # import global variables
        
        m1 = BOWLING_MASS # assign masses to global constants
        m2 = BOWLING_MASS
        
        m1StatusMsg.config(text = ("The mass of object 1 is",m1,"kg.")) # update status messages of masses
